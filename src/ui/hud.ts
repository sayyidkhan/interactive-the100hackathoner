...
export function createHud(
  ...
  let lastWaypointKey: string | null = null;
  let lastTownMapKey: string | null = null;
  ...
  const renderTownMap = (discovered: Set<string>) => {
    const mapKey = `${activeWaypointId ?? "none"}:${activeWaypointTracked}:${DISCOVERIES.map((entry) => discovered.has(entry.id) ? "1" : "0").join("")}`;
    if (mapKey === lastTownMapKey) return;
    lastTownMapKey = mapKey;
  ...
  const api: HudApi = {
    setPrompt(entry) {
      const unlocked = entry ? currentDiscovered.has(entry.id) : false;
      const promptKey = `${entry?.id ?? "none"}:${unlocked}`;
      if (promptKey === lastPromptKey) return;
      lastPromptKey = promptKey;
      prompt.hidden = !entry;
      promptText.textContent = entry ? `Space or E to read ${entry.title}` : "Space or E to read";
    ...
    setWaypoint(entry, distance = 0, heading = "center", tracked = false) {
      if (!entry) {
        if (lastWaypointKey === "complete") return;
        lastWaypointKey = "complete";
      ...
      const waypointKey = `${entry.id}:${roundedDistance}:${heading}:${tracked}:${arrived}:${routeProgress}`;
      if (waypointKey === lastWaypointKey) return;
