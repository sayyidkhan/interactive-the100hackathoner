...
export function initLoopTown(root: HTMLElement, options: LoopTownOptions = {}): void {
  ...
  const playerMotion: PlayerMotion = {
    verticalVelocity: 0,
    grounded: true,
    facingAngle: 0,
    walkTime: 0,
  ...
  const animate = () => {
    ...
    const gameplayActive = started && !builderActive && !cinematic.active && !hud.isModalOpen() && !cardOpen;
    if (gameplayActive) {
      ...
      if (!climbing) {
        updatePlayerMovement(player, playerMotion, input, delta, colliders, camera, updatePlayerRig);
      }
    ...
    if (builderActive) {
      ...
      updateCinematicExit(camera, cinematic, player.position, lookState, now, delta);
    } else if (waterTowerClimb.mode === "platform" && waterTowerAnchor) {
      updateWaterTowerCamera(camera, waterTowerAnchor);
    } else {
      updateGameplayCamera(camera, player.position, lookState, delta);
    }
